...
VBOMesh::VBOMesh( QOpenGLContext* qglHandle, const char * fileName, bool center, bool loadTc, bool genTangents) :
        reCenterMesh(center), loadTex(loadTc), genTang(genTangents)
{
    m_context = qglHandle;
bool ok = m_context->isValid();
QSurface* surface = m_context->surface();
...
m_context->create();
ok = m_context->makeCurrent(surface);
    ...
    ok = m_funcs->initializeOpenGLFunctions();
    m_funcs->glFlush();
    loadOBJ(fileName);
}
...
void VBOMesh::render() const {
    m_funcs->glBindVertexArray(vaoHandle);
        ...
        vector<vec4> & tangents)
{
    ...
    for( uint i = 0; i < faces.size(); i += 3 )
    {
        ...
        vec3 q1 = p2 - p1;
        vec3 q2 = p3 - p1;
        float s1 = tc2[0] - tc1[0], s2 = tc3[0] - tc1[0];
        float t1 = tc2[1] - tc1[1], t2 = tc3[1] - tc1[1];
                        ...
                        const vector<int> &elements )
{
    ...
    for( int i = 0; i < nVerts; ++i )
    {
        ...
        idx += 3;
        if( tc != NULL ) {
            tc[tcIdx] = texCoords[i][0];
            tc[tcIdx+1] = texCoords[i][1];
            tcIdx += 2;
        }
        if( tang != NULL ) {
    ...
    m_funcs->glGenVertexArrays( 1, &vaoHandle );
    m_funcs->glBindVertexArray(vaoHandle);
    ...
    uint handle[5];
    uint bufIdx = 0;
    m_funcs->glGenBuffers(nBuffers, handle);

    m_funcs->glBindBuffer(GL_ARRAY_BUFFER, handle[bufIdx++]);
    m_funcs->glBufferData(GL_ARRAY_BUFFER, (3 * nVerts) * sizeof(float), v, GL_STATIC_DRAW);
    m_funcs->glVertexAttribPointer( (GLuint)0, 3, GL_FLOAT, GL_FALSE, 0, ((GLubyte *)NULL + (0)) );
    m_funcs->glEnableVertexAttribArray(0);  // Vertex position

    m_funcs->glBindBuffer(GL_ARRAY_BUFFER, handle[bufIdx++]);
    m_funcs->glBufferData(GL_ARRAY_BUFFER, (3 * nVerts) * sizeof(float), n, GL_STATIC_DRAW);
    m_funcs->glVertexAttribPointer( (GLuint)1, 3, GL_FLOAT, GL_FALSE, 0, ((GLubyte *)NULL + (0)) );
    m_funcs->glEnableVertexAttribArray(1);  // Vertex normal

    if( tc != NULL ) {
        m_funcs->glBindBuffer(GL_ARRAY_BUFFER, handle[bufIdx++]);
        m_funcs->glBufferData(GL_ARRAY_BUFFER, (2 * nVerts) * sizeof(float), tc, GL_STATIC_DRAW);
        m_funcs->glVertexAttribPointer( (GLuint)2, 2, GL_FLOAT, GL_FALSE, 0, ((GLubyte *)NULL + (0)) );
        m_funcs->glEnableVertexAttribArray(2);  // Texture coords
    }
    if( tang != NULL ) {
        m_funcs->glBindBuffer(GL_ARRAY_BUFFER, handle[bufIdx++]);
        m_funcs->glBufferData(GL_ARRAY_BUFFER, (4 * nVerts) * sizeof(float), tang, GL_STATIC_DRAW);
        m_funcs->glVertexAttribPointer( (GLuint)3, 4, GL_FLOAT, GL_FALSE, 0, ((GLubyte *)NULL + (0)) );
        m_funcs->glEnableVertexAttribArray(3);  // Tangent vector
    }

    m_funcs->glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, handle[elementBuffer]);
    m_funcs->glBufferData(GL_ELEMENT_ARRAY_BUFFER, 3 * faces * sizeof(unsigned int), el, GL_STATIC_DRAW);
